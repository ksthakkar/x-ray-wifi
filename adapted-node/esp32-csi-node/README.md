...
## Classic ESP32 port (this tree)

This copy of the firmware is retargeted for classic ESP32 (PlatformIO
`board = esp32dev`) instead of ESP32-S3/ESP32-C6. It preserves the reference
firmware unchanged — same CSI streaming, edge DSP (breathing/heart
rate/presence/fall detection), WASM3 programmable sensing, OTA, mmWave
fusion, and mesh/adaptive-controller plane — with only the target-specific
pieces that don't apply to classic ESP32 disabled:

- **AMOLED display disabled** (`CONFIG_DISPLAY_ENABLE` unset in
  [`sdkconfig.defaults`](sdkconfig.defaults)) — the display needs QSPI pins
  (including GPIO 47/48) that don't exist on classic ESP32, plus PSRAM
  framebuffers most classic ESP32 boards lack.
- **Onboard WS2812 LED gamma-viz disabled** — the default GPIO (48) doesn't
  exist on classic ESP32 (valid range is 0-39), and most classic ESP32 dev
  boards have no onboard addressable LED anyway. `main.c` sets `led_gpio =
  -1` on this target as a safety net even if the feature is re-enabled.
- No other code changes: the CSI capture path (`csi_collector.c`) already
  used the same legacy (non-HE) `wifi_csi_config_t` that classic ESP32
  requires — that struct is shared with ESP32-S3 and required zero changes.
  All ESP32-C6-only modules (`c6_*.c`) are already gated by
  `CONFIG_IDF_TARGET_ESP32C6` and no-op automatically here.

Unlike the multi-target reference tree, this is a single-target port: all
classic-ESP32 settings (target, 4MB flash/partitions, disabled display/LED)
are baked directly into `sdkconfig.defaults` rather than a layered overlay,
because PlatformIO only reliably auto-applies a file with that exact name.
[`sdkconfig.defaults.esp32-classic`](sdkconfig.defaults.esp32-classic) is
kept alongside as the equivalent explicit two-file overlay for anyone
building with plain `idf.py` instead of PlatformIO.
