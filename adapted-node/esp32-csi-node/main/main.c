...
void app_main(void)
{
     ...
     * Classic ESP32 has no onboard addressable LED and GPIO 48 doesn't exist
     * there (valid range 0-39) — led_gpio=-1 disables the feature.
...
#if CONFIG_LED_GAMMA_VIZ
    if (led_gpio >= 0 &&
        led_strip_new_rmt_device(&strip_config, &rmt_config, &s_viz_led) == ESP_OK) {
    ...
    if (led_gpio >= 0 &&
        led_strip_new_rmt_device(&strip_config, &rmt_config, &led_strip) == ESP_OK) {
